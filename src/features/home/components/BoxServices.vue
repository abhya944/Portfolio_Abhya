...
const SERVICES_EN = [
  { name: "CI/CD", items: ["Jenkins", "Azure DevOps"] },
  { name: "Containers", items: ["Docker", "Kubernetes"] },
  { name: "Cloud & IaC", items: ["AWS", "Terraform"] },
  { name: "Automation", items: ["Ansible", "Python", "PowerShell"] },
  { name: "Monitoring", items: ["Prometheus", "Grafana"] },
  { name: "Data & OS", items: ["MySQL", "Linux/Windows"] },
...
const SERVICES_JA = [
  { name: "CI/CD", items: ["Jenkins", "Azure DevOps"] },
  { name: "コンテナ", items: ["Docker", "Kubernetes"] },
  { name: "クラウド/IaC", items: ["AWS", "Terraform"] },
  { name: "自動化", items: ["Ansible", "Python", "PowerShell"] },
  { name: "監視", items: ["Prometheus", "Grafana"] },
  { name: "データ/OS", items: ["MySQL", "Linux/Windows"] },
] as const satisfies { name: string; items: string[] }[];

const services = computed(() => {
  if (locale.value === "ja") return SERVICES_JA;
  ...
  <ProjectedElement :point="point">
    <div ref="wrapperRef" class="box-services">
      <div class="box-services-content">
        ...
        <div class="box-services-grid">
          <div class="box-services-column" v-for="(service, index) in services" :key="service.name">
            <p class="box-services-column-title">
              <AppearingText
                :text="service.name"
                :steps="1"
                :duration="0.35"
            ...
            <ul class="box-services-column-list">
              <li class="box-services-column-item" v-for="item in service.items" :key="item">{{ item }}</li>
            </ul>
          </div>
        </div>
      </div>
